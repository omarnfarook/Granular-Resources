...
### Key Components
- **Sidebar** — Left column, always visible, lists all sections
- **TopBar** — Thin strip above right pane with title, status, copy, docs toggle, theme toggle
- **ComponentCard** — Wraps each asset with status badge, copy, docs, placeholder styling
- **CopyButton** — Clipboard with execCommand fallback
- **DocPanel** — Expandable inline documentation panel
- **StatusBadge** — Dev Ready / In Progress / Deprecated / Draft pill
...
- Sidebar is ALWAYS visible. Never hides, never collapses.
...
4. If it's a Component or Pattern:
   - Create or update the .tsx file in src/components/
   - The ComponentCard preview renders the real component

5. NEVER leave orphan placeholders. If the user says "here are ALL my buttons," replace ALL button placeholders.

6. If a Pattern uses Components that are still placeholders, inform the user: "This uses [X] which is still a placeholder. Should I build that too?"

7. Sidebar never changes structure during replacement. Items go from placeholder → real in place.
...
3. Create component file if needed
...
When the user provides brand assets, follow the Edit Path Map. Set placeholder: false. Verify tokens in index.css.
